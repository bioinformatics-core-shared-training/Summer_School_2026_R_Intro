...
Now we can use `separate_wider_delim()` to disentangle the components of the
sample name.
...
This session's exercises involve transforming two data sets in order to ask a specific
...
In this session's materials we demonstrated the restructuring of different
...
::: {.callout-tip .hint collapse="true" title="Hint"}
The column headers from new_sp_m014 onwards contain a superfluous
...
::: {.callout-tip .hint collapse="true" title="Hint"}
Separating two of the variables is a bit tricky because there is no
separator character; you could modify the variable before separating.
:::
...
::: {.callout-tip .hint collapse="true" title="Hint"}
We're not distinguishing between diagnoses and genders in this plot so you'll
need to sum the numbers of cases for each country, year and age group.
:::

::: {.callout-note .answer collapse="true" title="Answer"}
```{r}
#| label: ex_3_uk_time_series
who_tidy <- who_tidy |>
