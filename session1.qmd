...
* R integrates with other tools to generate manuscripts from your code.  This
  document (a Quarto document) is a case in point.  
...
Positron provides us with a friendly interface to the R statistical programming
language.  It's a sophisticated IDE (Integrated Development Environment) that is
built on top of Microsoft's Visual Studio Code. It has many features that make
it easier to write and run R code, including syntax highlighting and code
completion. In this course we will only explore a few of the features of
Positron.

When you first open Positron you will see a screen similar to the one below. At
the bottom of the screen is the "Panel", currently this shows theconsole window,
which is the actual interface for the R programme. Above the "Panel" is the 
"Editor" window; we will write our code here. On the left-hand side of the 
screen is the "Activity Bar" which provides access to various features of 
Positron. Hovering the mouse over the icons in the "Activity Bar" will show you
what each icon does. In this course we will only be using the "Explorer" icon
to open the folder in which we will be working and files that we will work on.

<img src="assets/imgs/session1/Positron_Start.png" alt="Positron screenshot" width="90%">


## Console

At the bottom of the screenwe find the console window. This is the actual
interface for the R programme. 

We can type commands directly into the console and run them by pressing
`Enter`.

<img src="assets/imgs/session1/Positron_Console.png" alt="Positron screenshot" width="60%">
...
There are other tabs on this pane but we shall not be covering these on this
course.

## The Explorer

As general rule, the first thing to do is to open the folder in which you wish
to work. In our case this will be the `Bioinformatics_Workshop` directory, which
contains the `data` directory with all the data files we will be using in this 
course. To do this, click on the "Open Folder" button (or use the `File` menu >>
`Open Folder`) and navigate to the `Bioinformatics_Workshop` directory. 

When you first open the `Bioinformatics_Workshop` directory, you will just see
the `data` directory.  If you click on the small triangle to the left of the
`data` directory, you will see all the files as below.

<img src="assets/imgs/session1/Positron_OpenFolder.png" alt="Positron screenshot" width="90%">

Note that once you have used Positron to open a folder, it will remember this
and automatically open the last folder you were working in when you next open
Positron. 

## The Editor

The Editor is a panel for creating, editing & running R scripts. Rather than
simply typing our commands directly into the console, we can write them here as
an "R script".  As you will see, a script is basically a text file that contains
R commands and comments to explain what the code does.

Currently the Editor panel just contains the "Welcome" tab. To create a new R
script click on the "New File" button on the "Welcome" tab or use `File` menu >>
`New File`.  This will open the dialogue shown below. 

<img src="assets/imgs/session1/Positron_NewFile.png" alt="Positron screenshot" width="90%">

In the dialogue box where it says "*Select File Type or Enter File Name...*"
type `My_First_Rscript.R` and press `Enter`.  Save the new file in the
`Bioinformatics_Workshop` directory.  You should now see the new file in the
Explorer pane and the Editor pane should now look like this:

<img src="assets/imgs/session1/Positron_NewScript.png" alt="Positron screenshot" width="90%">

This is a blank script file.  You can now type your R commands here.  

We send the commands typed here to the console by placing the cursor on the line
with the command and pressing `Ctrl + Enter` (`Cmd + Enter` on a Mac). You can
also send multiple lines to the console by highlighting them and pressing `Ctrl
+ Enter` (`Cmd + Enter`  

The advantage of using a script rather than typing directly into the console is
that our commands are not lost, we can save the text file for future use and it
is easier to make changes to our code.

<img src="assets/imgs/session1/Positron_Scripting.png" alt="Positron edit pane screenshot" width="50%">
...
As well as R scripts, there are many types of Positron document including Quarto
documents, which we have used to build the materials for this course. These can
provide interactive workbooks or web documents to name but a few possible
outcomes.

## Session Panel

Now go to the `View` menu and select `Variables`. This will open the "Session" 
Panel on the right of the screen. This contains two panels: "Variables" and "Plots".

The "Variables" panel will show you all the variables that you have created in
your R session and their values.

<img src="assets/imgs/session1/Positron_Variables.png" alt="Positron variable panel" width="90%">

Below the "Variables" panel is the "Plots" panel, which will display any graphs
you create in your R session.

There are a number of other tabs available alongside the "Session" panel. The
only one that we will look at in this course is the "Help" tab, which will
display the help pages for any R function you are using. 
...
even though they do have subtly different meanings.
...
the value of a variable called "my", however, neither of these exists, so it
throws an error.
...
[Coercion](https://adv-r.hadley.nz/vectors-chap.html#testing-and-coercion)
...
![](./assets/imgs/session1/vector_recycling.png){width=40%}
...
logical vector should be the same length as the vector from which we are
...
We could use "!=" to select all values not equal to 20, but to demonstrate the
...
:::: exercise
6. "month.name" is an in-built R vector
...
:::: exercise
7. Use the `sample()` function to simulate the rolling of a 6-sided die.
   ...
   this exercise.
    ...
    a. Count the number of sixes you get for 1000 rolls.
    b. What number would you expect to get? Calculate this in R.
    c. Re-run the simulation a few times by clicking on the green 'Run current
    chunk' button. What do you notice?
    d. Re-run the simulation with a much larger sample size. Check that the
    number of sixes you obtain is much closer to the expected value.
...
#| label: ex_7_set_seed
#| include: false
set.seed(101010) # for reproducibility 
```
...
Challenge 7b: What number would you expect?
...
Challenge 7c: Re-running the simulation gives a different number of sixes each
time, because each run draws a new random sample.
...
number_of_rolls / 6
rolls <- sample(six_sided_die, size = number_of_rolls, replace = TRUE)
sum(rolls == 6)
```

The proportion of sixes gets closer to the expected value as the number of
rolls increases.
...
:::: exercise
8. Simulate the rolling of a weighted six-sided die.
   ...
   and square. Let's say the probability of throwing a six is 0.1 while the
   other 5 numbers all have equal probability.

   Calculate the probability of throwing each of the other numbers and then,
   referring again to the help page for `sample()`, re-run the simulation using
...
# rep() saves us writing out prob_other five times
rolls <- sample(1:6,
