...
rows from the two tables. We specify the column to match on with the **`by`**
argument, giving the column name as a string.
...
match using differently-named columns with the help of the **`join_by()`**
function. This is illustrated below using another version of the `animal_diets`
data frame in which the first column is called `species`. Note that inside
`join_by()` the column names are written without quotes, just as we do elsewhere
in the tidyverse.
...
for example, a genus and a species. Here too we use `join_by()`, listing the
several columns to be used in the join operation.
