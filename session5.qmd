...
The `|>` operator is built into R itself, so it is available as soon as you
start R without having to load any packages.
...
NOTE: `|>` is often called the "native pipe" because it is part of the R
language. It requires R version 4.1 or later.

You will also come across a second pipe operator, **`%>%`**, in a great deal of
existing R code, in older books and tutorials, and in answers you find online.
That one comes from the **magrittr** package and becomes available when you load
the tidyverse. For everyday use of the sort we're doing on this course the two
behave the same way, so when you meet `%>%` you can read it as `|>`. We'll use
the native `|>` throughout.
:::
...
There is a discrepancy. Can you see what the problem is?
...
summarizing groups of observations in the next session.
...
This is very similar to a plot we created in the last session.
...
The variable(s) used for faceting are specified using `vars()`, which lets us
refer to columns by name without quotes, in the same way that we've been
selecting columns with `select()` and inside `across()`.
...
classification in the GATA3 vs ESR1 scatter plot we created in the last session. Let's
...
:::: exercise
**3. Convert the expression values for each of the genes into standardized
z-scores**
...
particularly if the expression values for those genes are on very different
scales. The expression values in our METABRIC data set are on a log~2~ scale
which helps to reduce the range of values, but another way of representing
expression measurements is to standardize them to produce z-scores.

Standardization of a set of measurements involves subtracting the mean from
each and dividing by the standard deviation. This produces values with a mean
...
::: {.callout-note .answer collapse="true" title="Answer"}
```{r}
#| label: ex_3_esr1_z_score
metabric_z_scores <- mutate(metabric,
                            ESR1_z_score = (ESR1 - mean(ESR1)) / sd(ESR1))
```
:::
...
Add another column containing a z-score for GATA3 and then create a scatter
plot of the z-scores of GATA3 against ESR1. Modify your plot to facet by the
PAM50 classification.

::: {.callout-note .answer collapse="true" title="Answer"}
```{r}
#| label: ex_3_gata3_z_score_plot
...
Standardize the expression values for all 8 genes in a single operation using
an anonymous function, overwriting their original values, and round the
resulting values to 3 significant figures.

::: {.callout-note .answer collapse="true" title="Answer"}
```{r}
#| label: ex_3_all_genes_z_scores
...
Check you have done this correctly by computing the mean and standard deviation
for each column, again using a single operation.

::: {.callout-note .answer collapse="true" title="Answer"}
```{r}
#| label: ex_3_check_all_genes
metabric_z_scores |>
    summarise(across(ESR1:PGR, list(mean = mean, sd = sd)))
```
:::
::::
...
The `starwars` data set provided by dplyr contains height (in cm) and mass (in
kg) for each character. Compute the body mass index (BMI) for each character
and display the 10 characters with the highest BMI, showing their name, height,
mass and BMI.

BMI is calculated as mass in kilograms divided by the square of the height in
metres.

::: {.callout-note .answer collapse="true" title="Answer"}
```{r}
#| label: ex_4_starwars_bmi
starwars |>
...
Jabba the Hutt is, unsurprisingly, something of an outlier.
:::
::::
