...
Positron, you may be asked to select one of several mirrors of the CRAN R package
...
Notice that rather than specifying `tibble` as the data structure of surveys,
...
inherits from which in simple terms means that `tibble` is a `data.frame` with a
...
and writing tibbles to tab-delimited and comma-separated value (CSV) files.
...
Once we have a final data table from our analysis, we most often will want to
save it to a file. The `write_csv()` and `write_tsv()` functions from the
`readr` package provide a convenient way to write out our data frames
as comma-separated value (CSV) or tab-delimited files respectively. These
are plain text files that can be opened in a text editor or spreadsheet program
such as Microsoft Excel or LibreOffice Calc. In each case the first line of the
file can contain the column names and each subsequent line contains the values
for each observation (row) in the table. The values are separated by either
commas or tab characters depending on the function used to write the file.
...
```{r}
#| label: ex_1
selectedPatients <- select(metabric, Cellularity, ER_status, HER2_status)
selectedPatients <- filter(selectedPatients,
                           Cellularity == "High" &
                               ER_status == "Positive" &
...
```{r}
#| label: ex_2
selectedPatients <- filter(metabric,
                           Chemotherapy == "NO" &
                               Radiotherapy == "NO" &
                               Survival_time > 240)
...
3. Print out a subset of the `starwars` tibble containing just the characters
   with blond hair and blue eyes.

   `starwars` is a data set provided by dplyr, so it is available as soon as you
   have run `library(tidyverse)`.

::: {.callout-tip .hint collapse="true" title="Hint"}
Use `table()` first to see how the hair and eye colours have been recorded —
there is more than one spelling of "blond".
:::

::: {.callout-note .answer collapse="true" title="Answer"}

First, look at what values these two columns actually take.

```{r}
#| label: ex_3_explore_colours
table(starwars$hair_color)
table(starwars$eye_color)
```

Note that both "blond" and "blonde" have been used, so we need to allow for
either.

```{r}
#| label: ex_3_filter_or
filter(starwars,
       (hair_color == "blond" | hair_color == "blonde") &
           eye_color == "blue")
```

The `%in%` operator is a tidier way of writing the first condition.

```{r}
#| label: ex_3_filter_in
filter(starwars, hair_color %in% c("blond", "blonde") & eye_color == "blue")
```
...
4. Convert the values for the heights of characters in the `starwars` tibble
   from centimetres to metres.

    a. What happened to the type of the `height` column?
    b. Convert the heights back to centimetres and see if you can work out how
    to change the column back to its original type.

::: {.callout-note .answer collapse="true" title="Answer"}

The `height` column starts out as an integer.

```{r}
#| label: ex_4_convert_to_metres
typeof(starwars$height)
starwars$height <- starwars$height / 100
starwars
```

Challenge 4a: dividing by 100 changed the type from integer to double, because
the result of the division is not a whole number.

```{r}
#| label: ex_4a_height_type
typeof(starwars$height)
```

Challenge 4b: multiplying back up by 100 restores the values, but the column is
still a double — the type does not change back on its own.

```{r}
#| label: ex_4b_back_to_centimetres
starwars$height <- starwars$height * 100
typeof(starwars$height)
```

We have to convert it back explicitly.

```{r}
#| label: ex_4b_as_integer
